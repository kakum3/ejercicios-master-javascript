...
function renderInventory(inventory) {
  ...
  for (var i = 0; i < inventory.length; i++) {
    ...
    for (var n = 0; n < inventory[i].shoes.length; n++) {
      ...
      sumPrice += inventory[i].shoes[n].price; //arr.push([inventory[i].name + ', ' + inventory[i].shoes[n].name + ', ' + inventory[i].shoes[n].price]);
    ...
    arr.push({
      ...
      averagePrice: sumPrice / currentInventory.shoes.length
    });
  } //return arr;
